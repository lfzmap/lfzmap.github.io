...
root = "/home/lfz/projects/ultrasound/datasets/mediscan-seg/FL/images"
val = os.listdir(root)
val = [os.path.join(root, x) for x in val]
val = val[:10]
...
input_images = torch.stack([sam.preprocess(x.to(device)) for x in imgs], dim=0)
